...
var envHost = process.env['MONGO_NODE_DRIVE_HOST'],
    envPort = process.env['MONGO_NODE_DRIVE_PORT'],
...
module.exports = {
  find: function(name, query, limit, callback){
    db.collection(name).find(query).sort({_id: -1})
  ...
  push: function(name, id, updateQuery, callback){
    db.collection(name).update(
      {_id: id}, {$push: updateQuery}, {safe: true}, callback);
  },
